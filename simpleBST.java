...
/**
 * The simpleBST class represents a symbol table of generic key-value pairs
 * using a binary search tree. recursive versions of get and put have been
 * renamed rGet and rPut to facilitate testing of your non-recursive versions
 * you may not use rPut or rGet as part of your 'toDo' solutions a size method
 * is also provided which you may use
 * 
 */
public class simpleBST<Key extends Comparable<Key>, Value> {
	 ...
	 * iokeys
	 * 
	 * Returns an Iterable containing the keys of the tree corresponding to an
	 * InOrder traversal
	 * 
	 * Hint: follow the approach outlined in class. use a helper function to carry
	 * out the traversal
	 * 
	 * 
	 * To Do 1
	...
	public Iterable<Key> ioKeys() {
		...
		if (root == null) {
			...
			qok = (Queue<Key>) ioKeysHelper(root);
		}

		return qok; // To do 1 complete this method
	}

	private Iterable<Key> ioKeysHelper(Node x) {
		Queue<Key> ans = new Queue();
	 ...
	 * put
	 * 
	 * Inserts the specified key-value pair into the binary search tree, overwriting
	 * the old value with the new value if the tree already contains the specified
	 * key.
	 * 
	...
	public int numNodesWithExactlyOneChild() {
		...
		int count = oneChildHelper(root);

		return count; // ToDo 3 complete this method
	}

	private int oneChildHelper(Node x) {

		if (x.left == null && x.right == null) {
			return 0;
		} else if ((x.left != null && x.right == null)) {

			return 1 + oneChildHelper(x.left);
		} else if ((x.left == null && x.right != null)) {
			return 1 + oneChildHelper(x.right);
		}

		else {

			return oneChildHelper(x.right) + oneChildHelper(x.left);
		}
	}
	 ...
	 * 
	 * Returns the number of nodes with depth == d Precondition: none
	 * 
	...
	private int depthHelper(Node x, int depth) {
		int count = 0;
		int realDepth = 0;
		...
		if (depth > 0 && realDepth == depth - 1) {
			...
			if (x.right != null) {
				++count;
			 	...
			 	if (x.left != null && x.right != null) {
				return depthHelper(x.left, depth - 1) + depthHelper(x.left, depth - 1);
			}

			if (x.left != null) {
				return depthHelper(x.left, depth - 1);
			}

			if (x.right != null) {
				return depthHelper(x.right, depth - 1);
			}
		}
		return count;
	 ...
	 * delete
	 * 
	 * deletes the key (&value) from the table if the key is present using the the
	 * *dual* of the Hibbard approach from the text. That is, for the two-child
	 * scenario, delete the node by replacing it with it's predecessor (instead of
	 * its successor)
	 * 
	 ...
	 * 
	 * if you complete this toDo, comment out or delete the print statement,
	...
	private void deleteHelper(Node t, Key key) {
		...
		if (t.left == null && t.right == null) {
			...
			deleteHelper(t.right, key);
		} else if (key.compareTo(t.key) < 0) {
			deleteHelper(t.left, key);
		}

		deleteHelper(t.right, t.key);

	}

	/*
	 * if (key.compareTo(x.left.key) == 0) { System.out.println(x.key); /*
	 * if(max(x.left) != null) { deleteMax(x.left); }
	 * 
	 * }
	 * 
	 * if (key.compareTo(x.left.key) < 0) { if (x.left != null) x = x.left; }
	 * 
	...
	private Value rGet(Node x, Key key) {
		...
		if (cmp < 0)
			return rGet(x.left, key);
		else if (cmp > 0)
			return rGet(x.right, key);
		else
			return x.val;
	}
	...
	private Node rPut(Node x, Key key, Value val) {
		...
		int cmp = key.compareTo(x.key);
		if (cmp < 0)
			x.left = rPut(x.left, key, val);
		else if (cmp > 0)
			x.right = rPut(x.right, key, val);
		else
			x.val = val;
		return x;
	}
	 ...
	 * ioKeysTests
	 * 
	 * parameters to testIoKeys 1: String of keys to add to symbol table in order
	 * left-to-right 2: expected result
	 */
	 ...
	 * putTests
	 * 
	 * parameters to testPut 1: String of keys to add to a symbol table in order
	 * left-to-right 2: String of vals corresponding to above keys 3,4: key,value to
	 * 'put' 5,6: String of keys,values expected in resulting table in order
	 * left-to-right
	 */
	 ...
	 * numNodesWithExactlyOneChildTests
	 * 
	 * parameters to testNumNodesWithExactlyOneChild 1: String of keys & values to
	 * add to a symbol table in order left-to-right 2: expected number of nodes with
	 * exactly one child
	 */
	 ...
	 * 
	 * parameters to testnumNodesAtDepth 1: String of keys & values to add to a
	 * symbol table in order left-to-right 2: depth to check for 2: expected number
	 * of nodes at the specified depth
	 */
	 ...
	 * deleteTests
	 * 
	 * parameters to testDelete 1: String of keys to add to a symbol table in order
	 * left-to-right 2: String of vals corresponding to above keys 3: key to be
	 * deleted 4,5: String of keys,values expected in resulting table in order
	 * left-to-right
	 */
	...
	/**
	 * from builds a BST using the author's version of put
	 */
	...
	/**
	 * testIokeys param keys: all substrings of keys of length 1 are added to the ST
	...
	/**
	 * testNumNodesWithExactlyOneChild param keys: all substrings of keys of length
	 * 1 are added to the ST param expected: the correct result for this input
	 * string
	 */
	...
	/**
	 * testnumNodesAtDepth param keys: all substrings of keys of length 1 are added
	 * to the ST param theDepth: the depth to check for param expected: the correct
	 * result for this input string and depth
	 */
	...
	/**
	 * testPut param keys: all substrings of keys of length 1 are added to the ST
	 * param vals: corresponding values for keys param pKey, pVal: the key-value
	 * pair to be inserted into the ST param exKeys, exVals: strings contained the
	 * expected ST contents after inserting pKey,pVal
	 * 
	 * this test not check for inserting a null value
	 */

	public static void testPut(String keys, String vals, String pKey, String pVal, String exKeys, String exVals) {
	...
	/**
	 * testDelete param keys: all substrings of keys of length 1 are added to the ST
	 * param vals: corresponding values for keys param key: the key to be deleted
	 * param exKeys, exVals: strings contained the expected ST contents after
	 * deleting key
	 * 
	 */
