 ...
 *  
 *  CSC 403 Winter 2020  version 1.0
 *  
 *************************************************************************/

public class CSC403LinProbHashST<K, V> {
	...
	public CSC403LinProbHashST() {
		this(INIT_CAPACITY);
	}

	// create linear proving hash table of given capacity
	@SuppressWarnings("unchecked")
	public CSC403LinProbHashST(int capacity) {
		M = capacity;
		keys = (K[]) new Object[M];
		vals = (V[]) new Object[M];
	}
	...
	private int hash(K key) {
		return (key.hashCode()/*/1000000000*/ & 0x7fffffff) % M;
	...
	private void resize(int capacity) {
		...
		CSC403LinProbHashST<K, V> temp = new CSC403LinProbHashST<>(capacity);
		for (int i = 0; i < M; i++) {
			if (keys[i] != null) {
	...
	public void put(K key, V val) {
		...
		int start = (i + M/3) % M;   // normal linear probing starting place
	...
	public static void putExperiment( double loadFactor, String file, boolean checkIntegrity) { 
		...
		for ( int expSize = 1000; expSize <= 10000; expSize+=1000) {
			...
			CSC403LinProbHashST<GpsTime, String> aHashST = new CSC403LinProbHashST<>(expTableSize);
